...
function TouristDashboard() {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      try {
        const res = await fetch("https://fsad-tourconnect-backend.onrender.com/api/auth/me", { credentials: "include" });
        if (res.ok) {
  ...
  const handleSaveProfile = () => {
    ...
    fetch("https://fsad-tourconnect-backend.onrender.com/api/users/update", {
      method: "PUT",
      credentials: "include",
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {filteredBookings.map((b, idx) => (
                      <div key={idx} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${b.status?.toLowerCase() === 'confirmed' ? 'bg-green-50 text-green-700 border-green-100' :
