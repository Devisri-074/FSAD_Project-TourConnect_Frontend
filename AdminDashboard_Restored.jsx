...
function AdminDashboard() {
  ...
  const [availableCities, setAvailableCities] = useState([]); // Γ£à CITY MANAGEMENT
  const [newCityName, setNewCityName] = useState("");
  const [customAttractions, setCustomAttractions] = useState({}); // Γ£à ATTRACTION MANAGEMENT
        ...
        {activeTab === "Overview" && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <StatCard title="Total Revenue" value={`Γé╣ ${(() => {
                 let total = 0;
                 allPlans.forEach(t => {
                    ...
                    <div className="space-y-4">
                       ...
                       <div className="grid grid-cols-1 gap-4 bg-blue-50/50 p-4 rounded-xl border border-blue-100 mb-2">
                          <div className="flex justify-between items-center whitespace-nowrap">
                            ...
                            <p className="text-sm text-blue-900 font-bold mt-1">Γé╣{bPrice}</p>
                          </div>
                       ...
                       <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                          <div>
                            ...
                            <p className="text-xs text-green-700 font-bold mt-1">Γé╣{hPrice}</p>
                          </div>
                          <div>
                            ...
                            <p className="text-xs text-green-700 font-bold mt-1">Γé╣{gPrice}</p>
                          </div>
                       </div>
                       <div className="mt-4 flex justify-between items-center bg-green-50 px-4 py-3 rounded-lg border border-green-200">
                          ...
                          <span className="text-2xl font-bold text-green-700">Γé╣{totalGross}</span>
                       </div>
                    </div>
                      ...
                      <div key={p.originalIndex} className="p-6 border border-gray-200 rounded-xl bg-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm hover:shadow-md transition">
                          <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-sm font-semibold">
                                  <span className="text-green-700 bg-green-50 px-2 py-1 rounded">Listed Price: Γé╣{p.price} / night</span>
                                  <span className="text-yellow-600">Initial Rating: Γ¡É {p.rating}</span>
                              </div>
                          </div>
